...
class AddressAutocomplete {
  ...
  _attachInputListeners(input) {
    ...
    // For multi-field without housenumber, attach verification check to street input on blur
    if (
      this.mode === "multi" &&
      !this.inputs.housenumber &&
      input === this.inputs.street
    ) {
      // Verification check moved to blur event below
    }
    ...
    input.addEventListener("blur", (e) => {
      ...
      // Trigger verification check on blur if in verification mode
      if (this.verificationMode) {
        if (
          (this.mode === "single" && input === this.input) ||
          (this.mode === "multi" &&
            !this.inputs.housenumber &&
            input === this.inputs.street)
        ) {
  ...
  _handleInput(event) {
    ...
    if (this.mode === "single" && this.verificationMode) {
      ...
      if (!currentValue) {
        ...
        // Verification now happens on blur
        return;
      } else {
        this._exitVerificationMode();
      }
    ...
    if (
      ...
      const currentValue = (this.inputs.street?.value || "").trim();
      const baseStreet = this.pendingVerificationAddress?.street || "";
  ...
  async _performVerificationCheck() {
    ...
    try {
      ...
      if (result.valid || result.exists) {
        ...
        if (this.mode == "multi" && this.inputs.postcode) {
          const newPostcode = result.postcode || postcode;
          if (this.inputs.postcode.value !== newPostcode) {
            this.inputs.postcode.value = newPostcode;
            this.inputs.postcode.classList.add(
              "address-autocomplete-highlight",
            );
            setTimeout(
              () =>
                this.inputs.postcode.classList.remove(
                  "address-autocomplete-highlight",
                ),
              1000,
            );
        ...
        if (this.mode == "multi" && this.inputs.city) {
          const newCity = result.city || city;
          if (this.inputs.city.value !== newCity) {
            this.inputs.city.value = newCity;
            this.inputs.city.classList.add("address-autocomplete-highlight");
            setTimeout(
              () =>
                this.inputs.city.classList.remove(
                  "address-autocomplete-highlight",
                ),
              1000,
            );
